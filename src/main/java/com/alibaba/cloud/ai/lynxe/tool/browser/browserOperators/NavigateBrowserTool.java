...
public class NavigateBrowserTool extends AbstractBrowserTool<NavigateBrowserTool.NavigateInput> {
	...
	public NavigateBrowserTool(BrowserUseTool browserUseTool, ToolI18nService toolI18nService) {
		super(browserUseTool);
		this.toolI18nService = toolI18nService;
	}

	@Override
	protected BrowserRequestVO toBrowserRequestVO(NavigateInput input) {
		BrowserRequestVO request = new BrowserRequestVO();
		request.setAction("navigate");
	...
	public ToolExecuteResult run(NavigateInput input) {
		...
		try {
			...
			return executeActionWithRetry(() -> new NavigateAction(browserUseTool).execute(toBrowserRequestVO(input)),
					"navigate");
		}
